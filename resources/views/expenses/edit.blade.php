                    ...
                    <form action="{{ route('expenses.update', $expense) }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                <label for="expense_classification_id" class="form-label">Classificação da Despesa <span class="text-danger">*</span></label>
                                <select class="form-select @error('expense_classification_id') is-invalid @enderror" 
                                        id="expense_classification_id" 
                                        name="expense_classification_id" 
                                        required>
                                    <option value="">Selecione uma classificação</option>
                                    @foreach($classifications as $classification)
                                        <option value="{{ $classification->id }}" {{ old('expense_classification_id', $expense->expense_classification_id) == $classification->id ? 'selected' : '' }}>
                                            {{ $classification->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('expense_classification_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        </div>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select class="form-select @error('fonte_id') is-invalid @enderror" 
                                    ...
                                    @foreach($fontes as $fonte)
                                        <option value="{{ $fonte->id }}" {{ old('fonte_id', $expense->fonte_id) == $fonte->id ? 'selected' : '' }}>
                                            {{ $fonte->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select class="form-select @error('bloco_id') is-invalid @enderror" 
                                    ...
                                    @foreach($blocos as $bloco)
                                        <option value="{{ $bloco->id }}" {{ old('bloco_id', $expense->bloco_id) == $bloco->id ? 'selected' : '' }}>
                                            {{ $bloco->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select class="form-select @error('grupo_id') is-invalid @enderror" 
                                    ...
                                    @foreach($grupos as $grupo)
                                        <option value="{{ $grupo->id }}" {{ old('grupo_id', $expense->grupo_id) == $grupo->id ? 'selected' : '' }}>
                                            {{ $grupo->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select class="form-select @error('acao_id') is-invalid @enderror" 
                                    ...
                                    @foreach($acoes as $acao)
                                        <option value="{{ $acao->id }}" {{ old('acao_id', $expense->acao_id) == $acao->id ? 'selected' : '' }}>
                                            {{ $acao->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('observation') is-invalid @enderror" 
                                      id="observation" 
                                      name="observation" 
                                      rows="3">{{ old('observation', $expense->observation) }}</textarea>
                            @error('observation')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadBlocos(fonteId) {
        ...
        fetch(`{{ url('expenses/categories/blocos') }}/${fonteId}`)
            .then(response => response.json())
            .then(blocos => {
                let html = '<option value="">Selecione um bloco</option>';
                blocos.forEach(bloco => {
                    const selected = bloco.id == {{ old('bloco_id', $expense->bloco_id) }} ? 'selected' : '';
    ...
    function loadGrupos(blocoId) {
        ...
        fetch(`{{ url('expenses/categories/grupos') }}/${blocoId}`)
            .then(response => response.json())
            .then(grupos => {
                let html = '<option value="">Selecione um grupo</option>';
                grupos.forEach(grupo => {
                    const selected = grupo.id == {{ old('grupo_id', $expense->grupo_id) }} ? 'selected' : '';
    ...
    function loadAcoes(grupoId) {
        ...
        fetch(`{{ url('expenses/categories/acoes') }}/${grupoId}`)
            .then(response => response.json())
            .then(acoes => {
                let html = '<option value="">Selecione uma ação</option>';
                acoes.forEach(acao => {
                    const selected = acao.id == {{ old('acao_id', $expense->acao_id) }} ? 'selected' : '';
    ...
    if (fonteId) {
        loadBlocos(fonteId);
        const blocoId = {{ old('bloco_id', $expense->bloco_id) }};
        if (blocoId) {
            loadGrupos(blocoId);
            const grupoId = {{ old('grupo_id', $expense->grupo_id) }};
            if (grupoId) {
                loadAcoes(grupoId);
            }
        }
